...
exports.create = function (type, config) {
    ...
    var dust = freshy('dustjs-linkedin', function (dust) {
        ...
        dust.onLoad = function (name, options, cb) {
            if (isAbsolute(name)) {
                ...
                options.view.lookup(file, options, function (err, filename) {
    ...
    engine = function engine(file, options, callback) {
        ...
        var renderOptions = {
            ...
            name: nameNoExt, // so that the initial render and further renders can use the same style of cache keying
